...
describe("EvolutionToast", () => {
  ...
  it("newStagePokemon에 artwork가 있으면 작은 스프라이트가 함께 렌더된다", () => {
    render(
      <EvolutionToast
        result={{
          evolved: true,
          branchChoicePending: false,
          newStagePokemon: { nameKo: "이상해풀", artwork: "https://example.com/ivysaur.png" },
        }}
      />
    );
    const sprite = screen.getByRole("img", { name: "이상해풀" });
    expect(sprite).toHaveAttribute("src", "https://example.com/ivysaur.png");
  });

  it("newStagePokemon에 artwork가 없으면 스프라이트 없이 렌더된다", () => {
    render(
      <EvolutionToast
        result={{
          evolved: true,
  ...
  it("branchChoicePending일 때 홈 안내 문구가 렌더되고 스프라이트는 없다", () => {
    render(
      <EvolutionToast
